								...
								<div key={item.id}>
									<div className="ListCategoryLine d-flex">
									    ...
									    <div className="updateANDdelete w-100 text-end">
									        <span className="material-icons text-danger" data-bs-toggle="modal" data-bs-target="#exampleModal" onClick={() => modalDeleteCategory(item.id)}>delete</span>
		                                    <span className="material-icons-outlined text-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" onClick={() => modalUpdateCategory(item.id)}>update</span>
									    </div>
									</div>
									<hr className="mb-2" />
								</div>
			  ...
			  <div className="modal-dialog">
			    <div className="modal-content">
			      <div className="modal-header">
			        <h5 className="modal-title" id="exampleModalLabel">{modalBody.title}</h5>
			        <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
			      </div>
			      <div className="modal-body modal-body-category">
			        {modalBody.content}
			      </div>
			      <div className="modal-footer">
			        <button type="button" className="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
			        <button type="button" className="btn btn-primary" data-bs-dismiss="modal" onClick={modalBody.type === 'delete' ? () => DeleteCategory() : () => UpdateCategory() }>{modalBody.btnSave}</button>
			      </div>
			    </div>
			  </div>
			</div>
