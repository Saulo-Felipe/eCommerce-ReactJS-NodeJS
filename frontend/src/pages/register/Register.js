...
export default function Register() {
  ...
  const [msgAlert, setMsgAlert] = useState({type: "", message: ""})
      ...
      if (inputValue.name.length === 0 || inputValue.email.length === 0 || inputValue.password.length === 0 || inputValue.passwordTwo.length === 0) {
        ...
        if (inputValue.password === inputValue.passwordTwo) {
          ...
          if (response.data.error) {
            ...
            var login = { email: inputValue.email, password: inputValue.password }
    ...
    <div className="container mb-5">
      <div className="RegisterForm">
        ...
        <form className="mt-3">
          ...
          <input autoComplete="off" type="password" id="password" className="form-control mb-2" placeholder="Sua senha" onChange={handleChangeInput} required/>

          <label htmlFor="password-two ">Repita a senha</label>
          <input autoComplete="off" type="password" id="password-two" className="form-control mb-2" placeholder="Repita a senha" onChange={handleChangeInput} required/>
